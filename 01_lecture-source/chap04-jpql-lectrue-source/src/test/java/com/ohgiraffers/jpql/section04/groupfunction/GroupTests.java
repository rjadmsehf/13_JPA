...
public class GroupTests {
    ...
    /* 필기
        ...
        단, 몇 가지의 주의사항이 존재한다.
        1. 그룹함수의 반환 타입은 결과 값이 정수이면 Long, 실수이면 Double 로 반환된다.    <- int Inter 아니라 Long 임 참고하자
        2. 값이 없는 상태에서 count 를 제외한 그룹 함수는 null 이 되고, count 만 0이 된다
        -> 따라서 반환 값을 담기 위해 선언하는 변수 타입을 기본자료형으로 하면
        -> 조회 결과를 언박싱 할 때 nullpoingterexception이 발생한다.
...
//        int menuPrice = 50000;
        Long minPrice = 50000L;   //필기 Long 타입은 뒤에 L 을 붙여줘야 한다.
