...
public class SimpleJPQLRepository {
    ...
    public Menu testSingleRowByTypedQuery() {
    ...
    public List<Menu> selectMultiRowByQuery() {
        String jpql = "SELECT m FROM section01Menu m";
        Query query = manager.createQuery(jpql);
        List resultMenuList = query.getResultList();
    ...
    public List<Integer> selectUseDistinct() {
        ...
        String jpql = "SELECT DISTINCT m.categoryCode FROM section01Menu m";  //DISTINCT =  중복 제거 나중에 알아보기
